...
async function main(): Promise<void> {
  ...
  const primitiveStats: PrimitiveStats[] = [];
  ...
  for (const primitive of primitiveStats) {
    console.log(
      `- mesh="${primitive.meshName}" primitive=${primitive.primitiveIndex} positions=${primitive.positionCount} indices=${primitive.indexCount} material="${primitive.materialName}"`,
    );
  }
}
