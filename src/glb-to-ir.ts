import { type Material, type Node as GltfNode, type Primitive, type Scene, type Texture } from '@gltf-transform/core';
...
type IRMaterial = {
  ...
  b: number;
  a: number;
  source: IRMaterialSource;
};

type IRMaterialSource = {
  hasBaseColorTexture: boolean;
  baseColorTextureName?: string;
  textureAverageColor?: TextureAverageColor;
  colorSource: 'baseColorFactor' | 'baseColorTextureAverage';
};
...
function typedArrayToNumberArray(array: ArrayLike<number> | null, label: string): number[] {
  if (!array) {
...
function colorFactorToByte(value: number): number {
  return Math.max(0, Math.min(255, Math.round(value * 255)));
}

type TextureAverageColor = {
  r: number;
  g: number;
  b: number;
  a: number;
...
function averageRgbaPixels(data: Uint8Array | Buffer): TextureAverageColor {
  let r = 0;
  let g = 0;
  let b = 0;
  let a = 0;
...
function getTextureAverageColor(texture: Texture | null): TextureAverageColor | undefined {
  if (!texture) {
    return undefined;
  }
...
function multiplyTextureColorByFactor(textureColor: TextureAverageColor, factor: [number, number, number, number]): TextureAverageColor {
  return {
...
function createSpatialTree(
  scene: Scene,
  nodeIndexByNode: Map<GltfNode, number>,
  itemByNode: Map<GltfNode, IRItem>,
  sourceFile: string,
): IRSpatialTreeItem {
  const createNode = (node: GltfNode): IRSpatialTreeItem => {
...
function convertYUpToZUpVector(x: number, y: number, z: number): [number, number, number] {
  return [x, y, z];
}

function convertYUpToZUpArray(values: number[], label: string): number[] {
...
function computeBoundingBox(positions: number[]): IRBoundingBox {
...
function createIrMaterial(material: Material, materialIndex: number): IRMaterial {
...
function createDefaultMaterial(): IRMaterial {
  return {
    id: 'material-default',
    name: 'Default',
    r: 255,
...
function createGeometryId(index: number): string {
  ...
  for (const [nodeIndex, node] of nodes.entries()) {
    ...
    for (const [primitiveIndex, primitive] of mesh.listPrimitives().entries()) {
      ...
      if (!geometryId) {
        ...
        const geometryStats: IRGeometryStats = {
