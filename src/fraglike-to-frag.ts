...
type FragLikeModel = {
  ...
  attributes?: Array<{ data: string[] }>;
  relations?: Array<{ data: string[] }>;
...
type FragLikeMeshes = {
  ...
  meshes_items: number[];
  samples: FragLikeSample[];
  representations: FragLikeRepresentation[];
  materials: FragLikeMaterial[];
  circle_extrusions: [];
  shells: FragLikeShell[];
...
type FragLikeShell = {
  points: Vec3[];
  profiles: Array<{ indices: number[] }>;
  holes: [];
  big_profiles: Array<{ indices: number[] }>;
  big_holes: [];
  type: 'NONE' | 'BIG';
...
function validateFragLike(model: FragLikeModel): void {
...
function renderedFacesToEnum(value: FragLikeMaterial['rendered_faces']): RenderedFaces {
  if (value === 'ONE') {
...
function strokeToEnum(value: FragLikeMaterial['stroke']): Stroke {
  if (value === 'DEFAULT') {
...
function representationClassToEnum(value: FragLikeRepresentation['representation_class']): RepresentationClass {
  if (value === 'SHELL') {
...
function shellTypeToEnum(value: FragLikeShell['type']): ShellType {
  if (value === 'NONE') {
...
function createTransformStruct(builder: flatbuffers.Builder, transform: FragLikeTransform): flatbuffers.Offset {
  return Transform.createTransform(
    ...
    transform.x_direction[2],
    transform.y_direction[0],
    transform.y_direction[1],
    transform.y_direction[2],
  );
}

function createTransformVector(builder: flatbuffers.Builder, transforms: FragLikeTransform[]): flatbuffers.Offset {
...
function createPointsVector(builder: flatbuffers.Builder, points: Vec3[]): flatbuffers.Offset {
...
function createSamplesVector(builder: flatbuffers.Builder, samples: FragLikeSample[]): flatbuffers.Offset {
...
function createRepresentationsVector(builder: flatbuffers.Builder, representations: FragLikeRepresentation[]): flatbuffers.Offset {
  ...
  for (let index = representations.length - 1; index >= 0; index--) {
    ...
    Representation.createRepresentation(
      builder,
      representation.id,
      representation.bbox.min[0],
      representation.bbox.min[1],
      representation.bbox.min[2],
      representation.bbox.max[0],
      representation.bbox.max[1],
      representation.bbox.max[2],
      representationClassToEnum(representation.representation_class),
    );
  }
  return builder.endVector();
}

function createMaterialsVector(builder: flatbuffers.Builder, materials: FragLikeMaterial[]): flatbuffers.Offset {
  ...
  for (let index = materials.length - 1; index >= 0; index--) {
    ...
    Material.createMaterial(
      ...
      material.b,
      material.a,
      renderedFacesToEnum(material.rendered_faces),
      strokeToEnum(material.stroke),
    );
  }
  return builder.endVector();
}

function createShellProfile(builder: flatbuffers.Builder, indices: number[]): flatbuffers.Offset {
...
function createBigShellProfile(builder: flatbuffers.Builder, indices: number[]): flatbuffers.Offset {
...
function createShell(builder: flatbuffers.Builder, shell: FragLikeShell): flatbuffers.Offset {
  ...
  return Shell.createShell(
    ...
    bigProfilesOffset,
    bigHolesOffset,
    shellTypeToEnum(shell.type),
    profilesFaceIdsOffset,
  );
}

function createShellsVector(builder: flatbuffers.Builder, shells: FragLikeShell[]): flatbuffers.Offset {
  return Meshes.createShellsVector(
    builder,
    shells.map((shell) => createShell(builder, shell)),
  );
}

function createAttributesVector(builder: flatbuffers.Builder, attributes: Array<{ data: string[] }>): flatbuffers.Offset {
...
function createMeshes(builder: flatbuffers.Builder, meshes: FragLikeMeshes): flatbuffers.Offset {
...
function createModelBuffer(fragLike: FragLikeModel): Uint8Array {
...
async function main(): Promise<void> {
  ...
  const fragLike = JSON.parse(await fs.readFile(inputPath, 'utf8')) as FragLikeModel;
