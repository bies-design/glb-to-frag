...
function getMeshIndex(meshes, mesh) {
    return mesh ? meshes.indexOf(mesh) : null;
}
function getMaterialIndex(materials, material) {
    return material ? materials.indexOf(material) : null;
}
function getPrimitiveAttributes(primitive) {
...
async function readGlbJson(inputPath) {
...
function findKeywordHits(jsonText) {
...
function getRawExtensionNames(value) {
    return value?.extensions ? Object.keys(value.extensions).sort() : [];
}
function buildNodeHierarchy(rawJson, nodeReports) {
    const childNodeIndexes = new Set();
...
async function main() {
    ...
    const nodeReports = nodes.map((node, nodeIndex) => {
        ...
        const primitives = mesh?.listPrimitives().map((primitive, primitiveIndex) => {
    ...
    const textureReports = textures.map((texture, textureIndex) => {
